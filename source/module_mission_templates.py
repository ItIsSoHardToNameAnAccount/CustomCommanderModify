    ...
    [
      (try_for_agents, ":agent_no"),
        ...
        (try_begin),
          ...
          (else_try), # lefthand skill(skl_shield) to increase two_handed_wpn damage, 4% per level
            ...
            (try_begin), 
              ...
              (le, ":horse_agent", -1), # no horse
              (store_skill_level, ":skill_level", "skl_shield", ":troop_no"),
              (store_mul, ":damage_modifier", ":skill_level", 4),
              (val_add, ":damage_modifier", 100),
            (try_end),
          (else_try), # lefthand skill(skl_shield) to increase polearm damage, 4% per level
            (eq, ":item_type", itp_type_polearm),
            (agent_get_animation, ":cur_animation", ":agent_no", 1), # 1 = upper
            (try_begin), 
              (neg|is_between, ":cur_animation", "anim_ready_slashright_onehanded", "anim_blocked_slashright_onehanded"), 
              (neg|is_between, ":cur_animation", "anim_ready_slashleft_onehanded", "anim_blocked_slashleft_onehanded"), 
              (store_skill_level, ":skill_level", "skl_shield", ":troop_no"),
              (store_mul, ":damage_modifier", ":skill_level", 4),
              (val_add, ":damage_modifier", 100),
            (try_end),
...
bow_with_shield = (ti_on_item_wielded, 0, 0, [],
    [
      (store_trigger_param_1, ":agent_no"),
      #(store_trigger_param_2, ":item_no"),
      
      (set_fixed_point_multiplier, 1000),
      (agent_get_wielded_item, ":cur_weapon", ":agent_no", 0), # right hand
      (agent_get_wielded_item, ":cur_shield", ":agent_no", 1), # left hand
      (try_begin),
        (gt, ":cur_weapon", -1),
        (item_get_type, ":item_type", ":cur_weapon"),
        (eq, ":item_type", itp_type_bow),
        (gt, ":cur_shield", -1), # shield
        (item_get_weight, ":cur_shield_weight", ":cur_shield"),
        (gt, ":cur_shield_weight", 2000),
        (agent_set_wielded_item, ":agent_no", -1, 1),
        #(agent_set_wielded_item, ":agent_no", ":cur_weapon"),
      (try_end),
    ])
   
kill_message_with_icon = (ti_on_agent_killed_or_wounded, 0, 0, [],
       [
        (store_trigger_param, ":dead_agent_no", 1),
        (store_trigger_param, ":killer_agent_no", 2),
        (store_trigger_param, ":is_wounded", 3),
        (store_trigger_param, ":hit_bone", 4),
        (assign, ":killer_weapon", reg0),
        
        (try_begin),
          (agent_is_human, ":dead_agent_no"),
          (agent_is_human, ":killer_agent_no"),
          (call_script, "script_get_fight_icon", ":killer_agent_no", ":killer_weapon", ":hit_bone"),
          (assign, ":dest_icon", reg0),
          (this_or_next|is_presentation_active, "prsnt_mini_map"),
          (is_presentation_active, "prsnt_mini_map_bar"),
          (call_script, "script_update_kill_message_with_icon", ":killer_agent_no", ":dead_agent_no", ":dest_icon", ":is_wounded"),
        (try_end),
       ])
...
custom_commander_cheat_keys = (
  0, 0, 0, [],
  [
  # Ctrl+A(mmo): refill player ammo
  (get_player_agent_no, ":player_agent"),
  (try_begin), # 
    (key_is_down, key_left_control),
    (key_clicked, key_a),
    (agent_refill_ammo, ":player_agent"),
    (display_message, "@CHEAT! Your ammo gets refilled.", 0xFF0000),
  # Ctrl+S(hield): refill player shield HP
  (else_try),
    (key_is_down, key_left_control),
    (key_clicked, key_s),
    (agent_refill_wielded_shield_hit_points, ":player_agent"),
    (display_message, "@CHEAT! Your shield's HP gets refilled.", 0xFF0000),
...
# custom_commander_toggle_charging = (
  # 0, 0, 0, [],
   # [
    # # (eq, "$g_horse_charging_for_player", 1), ## CC
    # (get_player_agent_no, ":player_agent"),
    # (agent_get_horse, ":horse_agent", ":player_agent"),
    # (ge, ":horse_agent", 0),
...
custom_commander_horse_speed = (
  ...
  (try_for_agents, ":horse_agent"),
    ...
    # (else_try),
      ...
      # # (eq, "$g_horse_charging_for_ai", 1), ## CC
      # (agent_slot_eq, ":horse_agent", slot_agent_horse_is_charging, 0),
      # (store_agent_hit_points, ":horse_hp",":horse_agent"),
...
tournament_triggers = [
  ...
  (0, 0, 0, [],
    [
      (try_begin),
        (neg|is_presentation_active, "prsnt_only_kill_message_with_icon"),
        (start_presentation, "prsnt_only_kill_message_with_icon"),
      (try_end),
      ]),
  ...
  [
    ...
    (try_begin),
      ...
      (store_character_level, ":troop_level", ":dead_troop"),
    ...
    (try_begin),
      ...
      (call_script, "script_get_fight_icon", ":killer_agent_no", ":killer_weapon", ":hit_bone"),
      (assign, ":dest_icon", reg0),
      (is_presentation_active, "prsnt_only_kill_message_with_icon"),
      (call_script, "script_update_kill_message_with_icon", ":killer_agent_no", ":dead_agent_no", ":dest_icon", ":is_wounded"), 
    (try_end),
  ]),
      ...
      (ti_on_agent_killed_or_wounded, 0, 0, [],
       [
        ...
        (try_begin),
          ...
          # DEBUG message
          # (agent_get_attack_action, ":attack_action", ":killer_agent_no"),
          # (agent_get_action_dir, ":action_dir", ":killer_agent_no"),
          # (agent_get_animation, ":cur_animation", ":killer_agent_no", 1),
          # (assign, reg10, ":attack_action"),
          # (assign, reg11, ":action_dir"),
          # (assign, reg12, ":cur_animation"),
          # (display_message, "@attack_action : {reg10} ; action_dir : {reg11} ; cur_animation : {reg12}"), 
          # DEBUG message
        (try_end),
        ## CC
...
## CC
      ...
      # spawn player_agent horse, when player_agent is the attacker
      (0, 0, ti_once, [], 
        [
          (get_player_agent_no, ":player_agent"),
          (agent_get_team, ":player_team", ":player_agent"),
          (this_or_next|eq, ":player_team", "$attacker_team"),
          (eq, ":player_team", "$attacker_team_2"),
          (agent_get_troop_id, ":agent_troop", ":player_agent"),
          (agent_get_position, pos49, ":player_agent"),
          (position_move_x, pos49, -200),
          (set_spawn_position, pos49),
          (troop_get_inventory_slot, ":player_agent_horse_item", ":agent_troop", ek_horse),
          (troop_get_inventory_slot_modifier, ":player_agent_horse_item_modifier", ":agent_troop", ek_horse),
          (gt, ":player_agent_horse_item", -1),
          (spawn_horse, ":player_agent_horse_item", ":player_agent_horse_item_modifier"),
        ]),
