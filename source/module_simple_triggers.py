    ...
    [
     (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
       ...
       (try_for_range, ":unused", 0, ":num_hiring_rounds"),
         ...
         (troop_get_slot, ":accumulated_weekly_wage", ":troop_no", slot_troop_accumulated_weekly_wage),
         (store_div, ":cur_weekly_wage", ":accumulated_weekly_wage", 168),
...
## CC
  ...
  (14,
   [
    ...
    (store_mul, ":cooking_skill_bonus", ":cooking_skill", 3),
 ...
 #Update how good a target player is for bandits
  ...
  (72,
   [
     ...
     (call_script, "script_update_mercenary_units_of_reserves_party"),
     ## CC
     #NPC changes begin
   ...
   [
      ...
      (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
        ...
        (try_for_range, ":bandit_pt", "pt_looters", bandit_party_template_end),
          ...
          (this_or_next|eq, ":bandit_pt", "pt_dark_followers"),
  ...
  (3, #check to see if player's court has been captured
   [
     ...
     (try_for_parties, ":bandit_party"),
       ...
       (is_between, ":bandit_party_template", "pt_dark_followers", bandit_party_template_end), ## CC
       ...
       (try_begin),#If party is active and bandit is far away, then move to location
         (this_or_next|eq, ":bandit_lair", "p_dark_follower_spawn_point"), ## CC
    ...
    [
      (try_for_parties, ":party_no"),
        ...
        (else_try),
          ...
          (this_or_next|eq, ":party_template", "pt_dark_followers"),
  ...
  (24,
    [
      ...
      (try_for_range, ":cur_inactive_faction", commom_kingdoms_begin, ":end_cond"),
        ...
        (store_mul, ":restoration_days_limit", ":restoration_times", 30),
        (val_add, ":restoration_days_limit", 30),
        
        (gt, ":inactive_days", ":restoration_days_limit"),
  ...
  (24,
    [
      ...
      (try_for_range, ":rebel_faction", rebel_kingdoms_begin, ":end_cond"),
        ...
        (store_mul, ":rebellion_days_limit", ":rebellion_times", 30),
        (val_add, ":rebellion_days_limit", 30),
        
        (gt, ":inactive_days", ":rebellion_days_limit"),
        (faction_slot_eq, ":original_faction", slot_faction_state, sfs_active),
        (faction_get_slot, ":faction_leader", ":original_faction", slot_faction_leader),
   ...
   [
      (try_for_parties, ":party_no"),
        ...
        (try_for_range_backwards, ":cur_stack", 0, ":num_stacks"),
          ...
          (eq, ":troop_faction", "fac_dark_followers"),
          # (party_get_num_prisoner_stacks, ":num_prisoner_stacks", ":party_no"),
  ...
  (3,
   [
    (try_for_parties, ":party_no"),
      (party_get_template_id, ":party_template", ":party_no"),
      (try_begin),
        (is_between, ":party_template", "pt_dark_followers", bandit_party_template_end),
        (party_get_num_prisoner_stacks, ":num_prisoner_stacks", ":party_no"),
  ...
  (24*7, 
    [
      (try_for_parties, ":party_no"),
        ...
        (gt, ":member_rate_of_party", 50),
        (call_script, "script_party_remove_members_with_ratio", ":party_no", 50),
      (try_end),
   ]),
...
# # exchange cavalry troops in town/castle with non-cavalry troops in lord's party
 ...
 (1,
  ...
  # refresh center_reserves_party
  (1,
    [
      (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
        (party_get_slot, ":reserves_party", ":center_no", slot_center_reserves_party),
        # remove troops of other factions
        (call_script, "script_get_town_faction_for_recruiting", ":center_no"),
        (assign, ":cur_party_faction", reg0),
        (party_clear, "p_temp_party"), # ready to collect all fitable troops
        (set_spawn_radius, 2),
        (party_get_num_companion_stacks, ":num_stacks", ":reserves_party"),
        (try_for_range_backwards, ":i_stack", 0, ":num_stacks"),
          (party_stack_get_troop_id, ":stack_troop", ":reserves_party", ":i_stack"),
          (store_troop_faction, ":cur_troop_faction", ":stack_troop"),
          (is_between, ":cur_troop_faction", commom_kingdoms_begin, commom_kingdoms_end),
          (neq, ":cur_troop_faction", ":cur_party_faction"),
          (party_stack_get_size, ":stack_size", ":reserves_party", ":i_stack"),
          (party_remove_members, ":reserves_party", ":stack_troop", ":stack_size"),
          (party_add_members, "p_temp_party", ":stack_troop", ":stack_size"),
        (try_end),
        (try_begin),
          (party_get_num_companions, ":num_companions", "p_temp_party"),
        ...
        (party_slot_eq, ":center_no", slot_center_is_besieged_by, -1), # center not under siege
        # time
        (assign, ":hours_needed", 72),
        (try_begin), # shorten the time to 3/4
        ...
        (call_script, "script_get_num_mercenary_of_party", ":reserves_party"),
        (assign, ":num_mercenary", reg0),
        (store_sub, ":num_regular", ":num_men", ":num_mercenary"),
        # remove the half of regulars
        (try_begin), 
          (ge, ":num_regular", 30),
          (party_get_num_companion_stacks, ":num_stacks", ":reserves_party"),
          (try_for_range_backwards, ":stack_no", 0, ":num_stacks"),
            (party_stack_get_troop_id, ":stack_troop",":reserves_party",":stack_no"),
            (neg|is_between, ":stack_troop", mercenary_troops_begin, mercenary_troops_end),
            (party_stack_get_size, ":stack_size",":reserves_party",":stack_no"),
            (val_add, ":stack_size", 1),
            (val_div, ":stack_size", 2),
            (party_remove_members, ":reserves_party", ":stack_troop", ":stack_size"),
          (try_end),
        (try_end),
        # recruit_regular_units
        (call_script, "script_get_party_template_for_center_recruiting", ":center_no"),
        (assign, ":party_template", reg0),
        (party_add_template, ":reserves_party", ":party_template"),
        # reset last_recruit_time
        (party_set_slot, ":center_no", slot_town_last_recruit_time, ":cur_hours"),
        (call_script, "script_sort_party_by_troop_level", ":reserves_party"),
      (try_end),
    ]),
  
  # player's walled_center recruit reserves_party automatically
  (1,
    [
      (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
        (party_get_slot, ":reserves_party", ":center_no", slot_center_reserves_party),
        ...
        (try_for_range_backwards, ":i_stack", 0, ":num_stacks"),
          (party_stack_get_troop_id, ":stack_troop", ":reserves_party", ":i_stack"),
          # not mercenary troops or also_recruit_mercenary
          (this_or_next|neg|is_between, ":stack_troop", mercenary_troops_begin, mercenary_troops_end),
          (eq, "$g_also_recruit_mercenary", 1),
          (party_stack_get_size, ":stack_size", ":reserves_party", ":i_stack"),
          (call_script, "script_game_get_join_cost", ":stack_troop"),
          (assign, ":join_cost", reg0),
          (store_div, ":num_troops_can_recruit", ":recruit_gold", ":join_cost"), 
          (val_min, ":num_troops_can_recruit", ":stack_size"),
          (gt, ":num_troops_can_recruit", 0),
          # transfer members
          (party_add_members, ":center_no", ":stack_troop", ":num_troops_can_recruit"),
          (party_remove_members, ":reserves_party", ":stack_troop", ":num_troops_can_recruit"),
          # cost recruit_gold
          (store_mul, ":cur_stack_recruit_cost", ":join_cost", ":num_troops_can_recruit"), 
          (val_sub, ":recruit_gold", ":cur_stack_recruit_cost"),
        (try_end),
 ...
 # (6,
  ...
  (8,
   [
       (try_for_range, ":cur_travelling_horse_merchant", travelling_horse_merchants_begin, travelling_horse_merchants_end),
         (troop_get_slot, ":party_no", ":cur_travelling_horse_merchant", slot_troop_leaded_party),
         (party_is_in_any_town, ":party_no"),
         (party_get_cur_town, ":cur_center", ":party_no"),
         ...
         (try_begin), # refresh the horses of caravan
           ...
           (call_script, "script_refresh_horses_of_travelling_horse_merchant", ":cur_travelling_horse_merchant", ":cur_town_original_faction"),
           (call_script, "script_refresh_center_stables", ":cur_center", ":last_town_original_faction"),
           (try_begin),
  ...
  # weekly_wage_accumulated_value
  (1,
   [
     ## the player
     (call_script, "script_calculate_player_faction_wage"),
     (assign, ":cur_player_faction_wage", reg0),
     (troop_get_slot, ":accumulated_weekly_wage", "trp_player", slot_troop_accumulated_weekly_wage),
     (val_add, ":accumulated_weekly_wage", ":cur_player_faction_wage"),
     (troop_set_slot, "trp_player", slot_troop_accumulated_weekly_wage, ":accumulated_weekly_wage"),
     
     ## the lords: leaded_party wage
     (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
       (troop_slot_eq, ":troop_no", slot_troop_occupation, slto_kingdom_hero),
       (troop_get_slot, ":party_no", ":troop_no", slot_troop_leaded_party),
       (gt, ":party_no",0),
       ...
       (assign, ":cur_weekly_wage", reg0),
       (troop_get_slot, ":accumulated_weekly_wage", ":troop_no", slot_troop_accumulated_weekly_wage),
       (val_add, ":accumulated_weekly_wage", ":cur_weekly_wage"),
       (troop_set_slot, ":troop_no", slot_troop_accumulated_weekly_wage, ":accumulated_weekly_wage"),
     (try_end),
     
     ## the lords: walled_center wage
     (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),
       (party_get_slot, ":town_lord", ":center_no", slot_town_lord),
       (ge, ":town_lord", 1), #center does not belong to player and belongs to someone.
       (call_script, "script_calculate_weekly_party_wage", ":center_no"),
       (assign, ":cur_weekly_wage", reg0),
       (troop_get_slot, ":accumulated_weekly_wage", ":town_lord", slot_troop_accumulated_weekly_wage),
       (val_add, ":accumulated_weekly_wage", ":cur_weekly_wage"),
       (troop_set_slot, ":town_lord", slot_troop_accumulated_weekly_wage, ":accumulated_weekly_wage"),
     (try_end),
    ]),
